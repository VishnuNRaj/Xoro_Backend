...
const LoginRepository = (_a) => __awaiter(void 0, [_a], void 0, function* ({ Email, Password }) {
    try {
        ...
        if (user.Terminated) {
            return ResponseFunctions.LoginRes({
                errors: [],
                message: 'Sorry Account Terminated',
                status: 202
            });
        }
        const date = new Date();
        if (user.Suspended) {
            if (user.SuspendedTill != null && user.SuspendedTill < date) {
                const DateString = user.SuspendedTill.toLocaleDateString();
                return ResponseFunctions.LoginRes({
                    errors: [],
                    message: `Sorry Account Suspended Till ${DateString}`,
                    status: 202
                });
            }
            else {
                yield DatabaseFunctions.updateById(UnverifiedUsers_1.default, user._id, { Suspended: !user.Suspended, SuspendedTill: null });
            }
        }
...
const verifyUserAuthRepository = (token) => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const user = yield DatabaseFunctions.findUsingId(UnverifiedUsers_1.default, result.user.UserId);
        if (!user) {
            return ResponseFunctions.VerityAccountAuthRes({
                message: 'No User Found',
                status: 202,
        ...
        if (user.Suspended) {
            ...
            if (user.SuspendedTill && user.SuspendedTill > currentDate) {
                ...
                yield DatabaseFunctions.updateById(User_1.default, user._id, { Suspended: false });
                yield DatabaseFunctions.updateById(UnverifiedUsers_1.default, user._id, { suspended: false });
            }
        }
        console.log(user);
        return ResponseFunctions.VerityAccountAuthRes({
